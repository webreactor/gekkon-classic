...
работа с откомпилированными версиями происходит прозрачно
вызывается только файл шаблона
Класс Gekkon
подключение Gekkon
пример подключения -
...
эти свойства определяются в конструкторе следующим образом
```
$this->template_path=$path.'tpl/';
$this->bin_path=$path.'tpl_bin/';
$this->gekkon_path=$path.'Gekkon/';
```
где $path - параметр конструктора, полный путь к директории Gekkon

После создания обьекта - свойства можно переопределить 
